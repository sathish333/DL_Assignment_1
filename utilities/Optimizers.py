...
class Optimizers:
    def __init__(self,method):
        ...
        self.method=method # class variable to capture which method to use. 
    def update(self,obj,x_batch=None,y_batch=None):
        
        '''
        
        updates weights based on optimizer method.
        '''
        
        # fixing these hyper paramters values
        ...
        if self.method=='sgd':
            ...
            for i in range(len(obj.layers)):
                if obj.prev_gradients_w!=None: # if is first time
            ...
            # Nesterov Optimizer
            original_weights=copy.deepcopy(obj.weights) # to keep orignal weights as copy
            ...
            layer_outs,inter_values=obj.forward(x_batch) # forward propgate based on look ahead
            loss=0
            if(obj.loss_function==entropy_loss):
                loss=cross_entropy_loss(layer_outs[-1],y_batch)
            elif(obj.loss_function==squared_loss):
                loss=squared_loss(layer_outs[-1],y_batch)
                
              
            obj.compute_deltas(layer_outs,inter_values,y_batch)
            obj.find_gradients(x_batch,layer_outs) # find gradients 
            
            
            for i in range(len(obj.gradients_w)):
                if obj.prev_gradients_w!=None:
                    obj.gradients_w[i]=original_graidents_w[i]*momentum+lr*obj.gradients_w[i]
                    obj.gradients_b[i]=original_graidents_b[i]*momentum+lr*obj.gradients_b[i]
                    
                obj.weights[i]=original_weights[i]-(obj.gradients_w[i])
                obj.biases[i]=original_biases[i]-(obj.gradients_b[i])
